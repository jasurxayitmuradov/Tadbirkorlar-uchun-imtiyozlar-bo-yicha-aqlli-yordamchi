...
import courseSources from '../data/course_sources.json';
import { Card } from '../components/Card';
import { ShieldCheck, Database, GraduationCap, ExternalLink, RefreshCw } from 'lucide-react';
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {courseSources.map(source => (
            <Card key={source.id} className="border-l-4 border-l-purple-500 flex flex-col justify-between h-full">
               <div className="mb-4">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="font-bold text-white text-lg">{source.name}</h3>
                  {source.enabled && (
              ...
              <a 
                ...
                className="flex items-center justify-between p-2 rounded bg-slate-900/50 hover:bg-slate-900 text-slate-400 hover:text-white transition-colors text-sm"
              >
                Visit Provider <ExternalLink size={14} />
              </a>
            </Card>
          ))}
        </div>
